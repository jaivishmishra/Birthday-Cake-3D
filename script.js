...
// ===================================================================
// REALISTIC HAGRID 3D MODEL & PARCHMENT NAME TAG
// ===================================================================
let hagridGroup = null;

function makeHagridNameTagTex() {
    ...
    ctx.fillText('🧙‍♂️ RUBEUS HAGRID', 256, 64);

    return new THREE.CanvasTexture(cvs);
}

function buildHagrid() {
    hagridGroup = new THREE.Group();

    // High-visibility materials (emissive tint ensures Hagrid is ALWAYS bright & colorful!)
    const coatMat  = new THREE.MeshLambertMaterial({ color: 0x8B4513, emissive: 0x5C2E0B }); // Bright moleskin brown
    const trimMat  = new THREE.MeshLambertMaterial({ color: 0x50250A, emissive: 0x301505 }); // Coat trim
    const shirtMat = new THREE.MeshLambertMaterial({ color: 0xC0392B, emissive: 0x751E14 }); // Vibrant crimson tunic
    const beltMat  = new THREE.MeshLambertMaterial({ color: 0x2A150A, emissive: 0x1A0D05 }); // Belt
    const goldMat  = new THREE.MeshStandardMaterial({ color: 0xFFD700, emissive: 0x886600, metalness: 0.9, roughness: 0.2 }); // Gold Buckle
    const skinMat  = new THREE.MeshLambertMaterial({ color: 0xFFD8B8, emissive: 0x885533 }); // Bright skin tone
    const hairMat  = new THREE.MeshLambertMaterial({ color: 0x3D2418, emissive: 0x20120B }); // Wild beard/hair
    const bootMat  = new THREE.MeshLambertMaterial({ color: 0x221105, emissive: 0x110802 });

    // Boots
    [-0.55, 0.55].forEach(x => {
        const foot = new THREE.Mesh(new THREE.CapsuleGeometry(0.26, 0.55, 8, 12), bootMat);
        foot.rotation.x = Math.PI * 0.4;
        foot.position.set(x, 0.28, 0.2);
        hagridGroup.add(foot);
    });

    // Lower Flared Moleskin Coat
    const points = [];
    for (let i = 0; i <= 10; i++) {
        const t = i / 10;
        const radius = 0.85 + t * 0.8 + Math.sin(t * Math.PI) * 0.15;
        const y = t * 2.8;
        points.push(new THREE.Vector2(radius, y));
    }
    const coatLower = new THREE.Mesh(new THREE.LatheGeometry(points, 24), coatMat);
    coatLower.position.y = 0.4;
    hagridGroup.add(coatLower);

    // Torso
    const torso = new THREE.Mesh(new THREE.CylinderGeometry(1.25, 0.98, 1.6, 16), coatMat);
    torso.position.y = 2.4;
    hagridGroup.add(torso);

    // Open Coat Lapels & Crimson Tunic
    const tunic = new THREE.Mesh(new THREE.CylinderGeometry(0.58, 0.68, 1.5, 12), shirtMat);
    tunic.position.set(0, 2.35, 0.45);
    hagridGroup.add(tunic);

    [-0.45, 0.45].forEach((x, idx) => {
        const lapel = new THREE.Mesh(new THREE.BoxGeometry(0.34, 1.75, 0.2), trimMat);
        lapel.position.set(x, 2.45, 0.65);
        lapel.rotation.z = idx === 0 ? -0.18 : 0.18;
        hagridGroup.add(lapel);
    });

    // Belt & Gold Buckle
    const belt = new THREE.Mesh(new THREE.CylinderGeometry(1.04, 1.08, 0.26, 20), beltMat);
    belt.position.y = 1.7;
    hagridGroup.add(belt);

    const buckle = new THREE.Mesh(new THREE.BoxGeometry(0.44, 0.44, 0.15), goldMat);
    buckle.position.set(0, 1.7, 1.08);
    hagridGroup.add(buckle);

    // Head, Nose & Face
    const head = new THREE.Mesh(new THREE.SphereGeometry(0.46, 16, 16), skinMat);
    head.position.set(0, 3.68, 0.1);
    hagridGroup.add(head);

    const nose = new THREE.Mesh(new THREE.ConeGeometry(0.1, 0.26, 8), skinMat);
    nose.rotation.x = Math.PI * 0.52;
    nose.position.set(0, 3.68, 0.56);
    hagridGroup.add(nose);

    // Eyes
    [-0.18, 0.18].forEach(x => {
        const eye = new THREE.Mesh(new THREE.SphereGeometry(0.08, 8, 8), new THREE.MeshBasicMaterial({ color: 0xFFFFFF }));
        eye.position.set(x, 3.78, 0.46);
        const pupil = new THREE.Mesh(new THREE.SphereGeometry(0.045, 8, 8), new THREE.MeshBasicMaterial({ color: 0x111111 }));
        pupil.position.set(x, 3.78, 0.52);
        hagridGroup.add(eye, pupil);
    });

    // Bushy Beard & Wild Mane
    const beardGroup = new THREE.Group();
    for (let i = 0; i < 36; i++) {
        const b = new THREE.Mesh(new THREE.SphereGeometry(0.24 + Math.random() * 0.14, 10, 10), hairMat);
        const ang = (i / 36) * Math.PI * 1.5 - Math.PI * 0.75;
        const rad = 0.44 + Math.random() * 0.14;
        const hy  = 3.35 + (Math.random() - .5) * 0.9;
        b.position.set(Math.sin(ang) * rad, hy, Math.cos(ang) * rad + 0.14);
        beardGroup.add(b);
    }
    for (let i = 0; i < 28; i++) {
        const h = new THREE.Mesh(new THREE.SphereGeometry(0.32 + Math.random() * 0.16, 10, 10), hairMat);
        const ang = Math.random() * Math.PI * 2;
        const rad = 0.42 + Math.random() * 0.18;
        const hy  = 3.98 + Math.random() * 0.4;
        h.position.set(Math.cos(ang) * rad, hy, Math.sin(ang) * rad - 0.1);
        beardGroup.add(h);
    }
    hagridGroup.add(beardGroup);

    // Arms
    const armL = new THREE.Mesh(new THREE.CylinderGeometry(0.36, 0.3, 1.9, 12), coatMat);
    armL.position.set(-1.38, 2.4, 0.2);
    armL.rotation.z = Math.PI * 0.14;
    hagridGroup.add(armL);

    const armR = new THREE.Mesh(new THREE.CylinderGeometry(0.36, 0.3, 1.9, 12), coatMat);
    armR.position.set(1.38, 2.4, 0.2);
    armR.rotation.z = -Math.PI * 0.25;
    hagridGroup.add(armR);

    // Hands
    const handL = new THREE.Mesh(new THREE.SphereGeometry(0.2, 10, 10), skinMat);
    handL.position.set(-1.6, 1.4, 0.35);
    const handR = new THREE.Mesh(new THREE.SphereGeometry(0.2, 10, 10), skinMat);
    handR.position.set(1.7, 1.3, 0.7);
    hagridGroup.add(handL, handR);

    // ── 3D Floating Name Tag ──────────────────────────────────────
    const tagTex = makeHagridNameTagTex();
    const tagGeo = new THREE.PlaneGeometry(2.4, 0.6);
    const tagMat = new THREE.MeshBasicMaterial({ map: tagTex, side: THREE.DoubleSide, transparent: true });
    const nameTagMesh = new THREE.Mesh(tagGeo, tagMat);
    nameTagMesh.position.set(0, 5.2, 0);
    hagridGroup.add(nameTagMesh);

    // Dedicated Key Spotlight directly in front of Hagrid
    const hagridKeyLight = new THREE.PointLight(0xFFE0C0, 7.0, 22);
